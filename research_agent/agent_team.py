...
class LLMResearchTeam:
    ...
    def code(self, proposal: BroadProposal, *, failure: str = "") -> tuple[GeneratedCandidate, dict[str, Any]]:
        ...
        instruction += " Row-count rule: train_rows and validation_rows are sequences, never scalar counts. Use injected row_count(prepared, 'train'|'validation') for row counts; never call int(prepared.train_rows) or int(prepared.validation_rows)."
...
def build_isolated_candidate(source: str, workspace: Path) -> CandidateCallable:
    ...
    def row_count(prepared: Any, split: str) -> int:
        """Return a validated scalar row count; raw row sequences are not counts."""
        return int(feature_ids(prepared, split).shape[0])

    scope: dict[str, Any] = {"__builtins__": safe_builtins, "__name__": "isolated_candidate", "CandidateOutput": CandidateOutput, "validation_output": validation_output, "feature_ids": feature_ids, "binary_labels": binary_labels, "grouping_user_ids": grouping_user_ids, "row_count": row_count, "CPU_DEVICE": cpu_device, "torch": torch, "np": np}
