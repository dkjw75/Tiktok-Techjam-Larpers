...
- **Self-extending capabilities:** an LLM coding role may create an isolated candidate; a separate LLM verifier plus deterministic isolation checks must approve it before an append-only `capabilities.jsonl` record is created and training may begin.
